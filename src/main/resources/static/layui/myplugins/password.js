...
layui.define(['jquery','layer','util', 'form', 'laytpl', 'rate'], function(exports) {
	...
	var PWD_INPUT = [
		...
		'<!-- 密码 -->',
			...
			'</div>',
				...
				'<input type="password" id="{{ d.pwdid }}" name="{{ d.pwdName }}" required  lay-verify="required|password" placeholder="{{ d.pwd }}" autocomplete="off" maxLength="{{ d.length }}" class="layui-input">',
			'</div>',
		...
		'<div class="layui-input-inline">',
			...
			'<div class="layui-inline" style="width: 85%">',
				'<input type="password" id="{{ d.rpwdid }}" name="{{ d.checkPwdName }}" required  lay-verify="required|password" placeholder="{{ d.checkPwd }}" autocomplete="off" maxLength="{{ d.length }}" class="layui-input">',
			'</div>',
	...
	Pwd.prototype.security = function(options){
		...
		pwdInput.focus(function() {
		  options.tipsId = layer.tips(['请输入',options.minLength,'-',options.length,'个由数字,大小写字母或特殊符号组成的密码,以确保账号安全。'].join(''), this,{time:0});
	...
	Pwd.prototype.event = function(options){
		var verifyObj={};
		verifyObj[options.rpwdid]=function(value){
			if(value.length < 1){
			    return '请再输入一遍密码';
			}
